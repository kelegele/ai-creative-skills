...
**⚠️ 过筛要考虑选题形态(单期 vs 系列):** 一个内容"单期对小白太难"不等于该 reject——它可能适合放进**循序渐进的系列**(有铺垫,小白能跟上)。判据是"在合适形态下能不能服务受众",不是"单期塞得下才留"。不确定单期 / 系列时,**先问用户**,别擅自 reject。(教训:曾把"AI 编程四阶段"当单期 reject,实际用户要做成"大模型发展脉络系列"。)
